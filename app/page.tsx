...
export default function NewslineRadio() {
  ...
  useEffect(() => {
    const initializeUser = async () => {
      try {
        ...
        if (!sessionId) {
          // New session - increment listener count
    ...
    const handleBeforeUnload = (event) => {
      // Check if this is a reload or navigation within the same origin
      if (event.persisted || document.visibilityState === "visible") {
        return // Don't decrement on reload
      }
      decrementListener()
    }

    const handlePageHide = (event) => {
      // Only decrement if the page is being hidden permanently (not just minimized)
      if (event.persisted) {
        return // Don't decrement if page is being cached
      }
      decrementListener()
    }
    ...
    const handleVisibilityChange = () => {
      if (document.visibilityState === "hidden") {
        // Use a timeout to distinguish between tab switching and actual closing
        setTimeout(() => {
          if (document.visibilityState === "hidden") {
            decrementListener()
          }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-slate-800 text-white overflow-hidden">
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-40 -right-40 w-80 h-80 bg-blue-600/20 rounded-full blur-3xl animate-pulse"></div>
        <div className="absolute -bottom-40 -left-40 w-80 h-80 bg-red-600/15 rounded-full blur-3xl animate-pulse delay-1000"></div>
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-blue-500/10 rounded-full blur-3xl animate-spin-slow"></div>
        <div className="absolute bottom-1/4 right-1/4 w-64 h-64 bg-blue-700/15 rounded-full blur-2xl animate-bounce-slow"></div>

        <div className="absolute top-20 left-20 w-4 h-4 bg-blue-500 rounded-full animate-float"></div>
        <div className="absolute top-40 right-32 w-3 h-3 bg-red-400 rounded-full animate-float delay-500"></div>
        <div className="absolute bottom-32 left-1/3 w-2 h-2 bg-blue-600 rounded-full animate-float delay-1000"></div>
      </div>

      <div className="relative z-10">
        <header className="p-6 backdrop-blur-md bg-white/5 border-b border-blue-500/20 shadow-lg">
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            <div className="flex items-center space-x-6">
              <div className="relative group">
                <Link href="/admin">
                  <Image
                    ...
                    width={140}
                    height={70}
                    className="drop-shadow-lg group-hover:scale-105 transition-transform duration-300 cursor-pointer"
                  />
                </Link>
              </div>
              <div className="hidden md:flex items-center space-x-4">
                ...
                <div className="flex items-center space-x-2 text-sm">
                  ...
                  <span className="font-mono text-red-300">{listeners.toLocaleString()}</span>
            ...
            <div className="flex items-center space-x-4">
              <div className="hidden md:flex items-center space-x-4 text-sm">
                ...
                <span className="font-mono text-red-300">{currentTime.toLocaleTimeString()}</span>
              </div>
        ...
        <main className="max-w-7xl mx-auto p-6 space-y-16">
          <section className="text-center space-y-8">
            <div className="space-y-4 animate-fade-in">
              <h1 className="text-6xl md:text-8xl font-bold bg-gradient-to-r from-blue-500 via-blue-400 to-red-600 bg-clip-text text-transparent animate-pulse-glow">
                LIVE ON AIR
              </h1>
              <p className="text-xl text-gray-300 max-w-3xl mx-auto">
                Broadcasting the latest news, music, and entertainment 24/7 from Newline Radio
              </p>
            </div>

            <div className="grid md:grid-cols-3 gap-6">
              <div className="md:col-span-2">
                <Card className="backdrop-blur-lg bg-gradient-to-br from-blue-900/20 to-blue-800/10 border-blue-500/30 shadow-2xl animate-slide-up">
                  <CardContent className="p-8 space-y-6">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <h3 className="text-xl font-bold text-red-300">Morning Newsline</h3>
                          <p className="text-gray-400">with Sarah Johnson</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-2xl font-bold text-red-400">{listeners.toLocaleString()}</div>
                    ...
                    <div className="flex items-center justify-center space-x-6">
                      <Button
                        ...
                        variant="ghost"
                        size="lg"
                        className={`${isLiked ? "text-red-400" : "text-gray-400"} hover:text-red-300 transition-colors ${isLoading ? "opacity-50" : ""}`}
                      >
                        <Heart className={`w-6 h-6 ${isLiked ? "fill-current" : ""}`} />
                        <span className="ml-2">{isLoading ? "..." : likes}</span>
                      </Button>

                      <Button variant="ghost" size="lg" className="text-gray-400 hover:text-red-300">
                        <Share2 className="w-6 h-6" />
                      </Button>
                    </div>

                    <div className="space-y-4">
                      <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-slate-800/50 to-slate-900/50 p-4 border border-blue-500/20 shadow-inner">
                        ...
                        <iframe
                          ...
                          className="w-full min-h-[180px] border-0 rounded-lg bg-transparent relative z-10"
              ...
              <div className="space-y-4">
                <Card className="backdrop-blur-lg bg-white/5 border-blue-500/20">
                  ...
                  <CardContent className="space-y-4">
                    <div className="text-center">
                      <div className="w-16 h-16 bg-gradient-to-br from-red-500 to-red-700 rounded-full mx-auto mb-3 flex items-center justify-center animate-spin-slow">
                    ...
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-400">{formatTime(currentTrack.currentTime)}</span>
                      <span className="text-gray-400">{formatTime(currentTrack.duration)}</span>
                    </div>
                ...
                <Card className="backdrop-blur-lg bg-white/5 border-blue-500/20">
                  <CardContent className="p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-400">Peak Listeners Today</span>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <span className="text-sm text-red-400 ml-1">4.9</span>
                      </div>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-400">Total Likes</span>
                      <span className="text-red-400 font-bold">{isLoading ? "..." : likes.toLocaleString()}</span>
                    </div>
                  </CardContent>
                </Card>
              </div>
            ...
            {recentlyPlayed.length === 0 ? (
              ...
              <div className="grid gap-4 md:grid-cols-2">
                {recentlyPlayed.map((track, index) => (
                  <Card
                    key={index}
                    className="backdrop-blur-lg bg-white/5 border-red-500/10 hover:bg-white/10 transition-all duration-300 animate-fade-in-up"
